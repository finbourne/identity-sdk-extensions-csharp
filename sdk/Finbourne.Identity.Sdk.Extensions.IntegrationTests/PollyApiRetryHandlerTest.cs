...
namespace Finbourne.Identity.Sdk.Extensions.IntegrationTests
{
    ...
    public class PollyApiRetryHandlerTests
    {
        ...
        private readonly OAuthApplication _mockResponse =
            new OAuthApplication("id", "type", "displayName", "secret", "clientId", "issuer");
        ...
        public void CallApiMethod_WhenHttpStatusIs400AndRetryConditionIsNotSatisfied_ThrowsApiExceptionWithoutRetry()
        {
            ...
            var exception = Assert.Throws<ApiException>(
                () => _apiFactory.Api<ApplicationsApi>().GetApplication("app_does_not_exist")
            );
        ...
        public void CallApiMethod_WhenHttpStatusIs200AndRetryConditionIsNotSatisfied_NoPollyRetryIsTriggered()
        {
            ...
            var sdkResponse = _apiFactory.Api<ApplicationsApi>().GetApplication("app_does_not_exist");
            ...
            int returnedStatusCode)
        {
            ...
            // Calling GetApplication or any other API triggers the flow that triggers polly
            var exception = Assert.Throws<ApiException>(
                () => _apiFactory.Api<ApplicationsApi>().GetApplication("app_does_not_exist")
            );
            ...
            int returnedStatusCode)
        {
            ...
            // Calling GetApplication or any other API triggers the flow that triggers polly
            var response = _apiFactory.Api<ApplicationsApi>().GetApplication("app_does_not_exist");
        ...
        public void CallApiMethod_WhenApiResponseStatusCodeSatisfiesRetryCriteria_ConnectivityIssuesNoRetry_Throws()
        {
            ...
            Assert.That(exception.Message,
                Contains.Substring(
                    "Internal SDK error occurred when calling GetApplication: An error occurred while sending the request"));
        }
        ...
        public void UsePolicyWrap_WhenCallingApiMethodHitsRateLimit_BothDefaultAndRateLimitPoliciesAreUsed()
        {
            ...
            var sdkResponse = _apiFactory.Api<ApplicationsApi>().GetApplication("code");
        ...
        public void UseRateLimitPolicyNoRetryAfter_WhenCallingApiMethodHitsRateLimit_RetryUsesExponentialBackoff()
        {
            ...
            var sw = Stopwatch.StartNew();
            // Calling the API triggers the flow that triggers polly
            _apiFactory.Api<ApplicationsApi>().GetApplication("code");
            sw.Stop();
            ...
            CallApiMethodAsync_WhenHttpStatusIs200AndRetryConditionIsNotSatisfied_NoPollyRetryIsTriggered()
        {
            ...
            var sdkResponse = await _apiFactory.Api<IApplicationsApi>().GetApplicationAsync("any");
            ...
            int returnedStatusCode)
        {
            ...
            // Calling GetApplicationAsync or any other API triggers the flow that triggers polly
            var sdkResponse = await _apiFactory.Api<IApplicationsApi>().GetApplicationAsync("any");
            ...
            int returnedStatusCode)
        {
            ...
            var exception = Assert.ThrowsAsync<ApiException>(
                () => _apiFactory.Api<IApplicationsApi>().GetApplicationAsync("any"));

            Assert.That(exception.Message, Is.EqualTo($"Error calling GetApplication: {expectedErrorResponse}"));
            Assert.That(exception.ErrorCode, Is.EqualTo(returnedStatusCode));
            Assert.That(_apiCallCount, Is.EqualTo(expectedNumberOfApiCalls));
        }
                ...
                int returnedStatusCode)
        {
            ...
            // Calling GetApplicationAsync or any other API triggers the flow that triggers polly
            var response = await _apiFactory.Api<IApplicationsApi>().GetApplicationAsync("any");
        ...
        public async Task UsePolicyWrapAsync_WhenCallingApiMethodHitsRateLimit_BothDefaultAndRateLimitPoliciesAreUsed()
        {
            ...
            // Calling API triggers the flow that triggers polly
            var sdkResponse = await _apiFactory.Api<ApplicationsApi>().GetApplicationAsync("code");
            ...
            UseRateLimitPolicyNoRetryAfterAsync_WhenCallingApiMethodHitsRateLimit_RetryUsesExponentialBackoff()
        {
            ...
            var sw = Stopwatch.StartNew();
            // Calling API triggers the flow that triggers polly
            await _apiFactory.Api<ApplicationsApi>().GetApplicationAsync("code");
            sw.Stop();
            Assert.That(sw.ElapsedMilliseconds, Is.GreaterThanOrEqualTo(1000 * (2 + 4 + 8))); // exponential backoff
            Assert.That(_apiCallCount, Is.EqualTo(expectedNumberOfApiCalls));
        }
