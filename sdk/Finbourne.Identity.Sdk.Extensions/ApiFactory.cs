...
namespace Finbourne.Identity.Sdk.Extensions
{
    ...
    public class ApiFactory : IApiFactory
    {
        ...
        public ApiFactory(ApiConfiguration apiConfiguration)
        {
            ...
            if (!Uri.TryCreate(apiConfiguration.IdentityUrl, UriKind.Absolute, out var _))
            {
                if (string.IsNullOrWhiteSpace(apiConfiguration.IdentityUrl))
                    throw new ArgumentNullException(
                        nameof(apiConfiguration.IdentityUrl),
                        $"Identity Uri missing. Please specify either FBN_IDENTITY_API_URL environment variable or identityUrl in secrets.json.");

                throw new UriFormatException($"Invalid Uri: {apiConfiguration.IdentityUrl}");
            }

            // Create configuration
            var configuration = new TokenProviderConfiguration(tokenProvider)
            {
                BasePath = apiConfiguration.IdentityUrl,
            };
